    ...
    static createAsteroidBelt(ship: Ship, level: number): Asteroid[] {
        ...
        for (let i: number = 0; i < (3 + level); i++) {
            do {
                ...
                distBetweenPoints(ship.x, ship.y, x, y) < ((ASTEROID_SIZE * 2) + ship.radius)
            ) {
        ...
        if (asteroid) {
            ...
            if (this.score > this.highScore) {
                ...
                if (isLocalStorageAvailable()) {
                    window.localStorage.setItem(HIGH_SCORE_KEY, this.highScore.toString());
                }
            }
        ...
        return (
            <Wrapper>
                <Container>
                    ...
                    width={width} />
                    <StyledButton
                        onClick={() => this.props.closeAsteroids()}>
                        I Quit
                    </StyledButton>
                </Container>
            </Wrapper>
        ...
        if (current && this.ship) {
            ...
            for (let i: number = 0; i < this.lives; i++) {
                drawBasicShip(
                    ...
                    (isExploding && (i === this.lives - 1) ? palette.ui.red : palette.greyScale.white)
                );
            }
            ...
            for (let i: number = this.asteroids.length - 1; i >= 0; i--) {
                ...
                for (let j: number = this.ship.lasers.length - 1; j >= 0; j--) {
                    ...
                    if (
                        ...
                        this.explosions.push(new Explosion(this.ship.lasers[j].x, this.ship.lasers[j].y));
...
export default connect(null, mapDispatchToProps)(AsteroidsGame);
