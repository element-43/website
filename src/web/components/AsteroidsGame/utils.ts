...
export function drawAsteroid(ctx: CanvasRenderingContext2D, asteroid: Asteroid): void {
    ...
    for (let i: number = 1; i < asteroid.vertices; i++) {
        ctx.lineTo(
            asteroid.x + asteroid.radius * asteroid.offsets[i] * Math.cos(asteroid.angle + i * Math.PI * 2 / asteroid.vertices),
            asteroid.y + asteroid.radius * asteroid.offsets[i] * Math.sin(asteroid.angle + i * Math.PI * 2 / asteroid.vertices)
        );
    }
    ctx.closePath();
    ctx.stroke();
...
export function drawBasicShip(ctx: CanvasRenderingContext2D, entity: Entity, colour?: string): void {
    ...
    ctx.lineTo( // Rear left.
        entity.x - entity.radius * (2 / 3 * Math.cos(entity.angle) + Math.sin(entity.angle)),
        entity.y + entity.radius * (2 / 3 * Math.sin(entity.angle) - Math.cos(entity.angle))
    );
...
export function drawHighScore(ctx: CanvasRenderingContext2D, highScore: number, width: number): void {
...
export function drawSpace(ctx: CanvasRenderingContext2D, height: number, width: number): void {
    ctx.fillStyle = palette.greyScale.black;
    ctx.fillRect(0, 0, width, height);
}

export function drawScore(ctx: CanvasRenderingContext2D, score: number, width: number): void {
    ctx.textAlign = 'right';
    ctx.textBaseline = 'middle';
    ctx.fillStyle = palette.greyScale.white;
...
export function moveAsteroids(asteroids: Asteroid[], height: number, width: number): void {
...
export function moveLasers(lasers: Laser[], height: number, width: number): void {
